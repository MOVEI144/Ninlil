...
static int profile_within(const ninlil_role_profile *profile,
                          const ninlil_role_profile *limit)
{
    return profile->active_peers <= limit->active_peers &&
           profile->provisional_peers <= limit->provisional_peers &&
           profile->max_outbound <= limit->max_outbound &&
           profile->max_inbound <= limit->max_inbound &&
           profile->relay_custody <= limit->relay_custody &&
           profile->dedupe_ids <= limit->dedupe_ids &&
           profile->service_grants <= limit->service_grants &&
           profile->dram_ceiling_bytes <= limit->dram_ceiling_bytes &&
           profile->flash_ceiling_bytes <= limit->flash_ceiling_bytes;
}
...
int ninlil_role_profile_validate(const ninlil_role_profile *profile)
{
    ...
    if (!profile || profile->profile_version != PROFILE_VERSION ||
        ...
        !profile_within(profile, &limit) || profile->active_peers == 0u ||
    ...
    if (admission_total != profile->max_outbound ||
        ...
        (profile->max_total_owned != 0u &&
         (profile->max_total_owned > limit.max_total_owned ||
