...
#include <string.h>
static int context_equal(const ninlil_link_context *a, const ninlil_link_context *b)
{
...
static int same(const ninlil_link_window *a, const ninlil_link_window *b)
{
    return context_equal(&a->context, &b->context) &&
           a->first_sequence == b->first_sequence && a->last_sequence == b->last_sequence &&
           a->first_tx_ms == b->first_tx_ms && a->closed_ms == b->closed_ms &&
           a->airtime_sum_us == b->airtime_sum_us && a->queue_max_us == b->queue_max_us &&
           a->attempts == b->attempts && a->delivered == b->delivered;
}
int ninlil_power_policy_open(ninlil_power_policy *p, const ninlil_link_context *c,
                              int8_t minimum, int8_t maximum, uint64_t now)
{
    if (!p || !c || !c->generation || !c->profile || minimum < -9 || maximum > 22 ||
        minimum > maximum || (maximum-minimum)%3 || c->power_dbm != maximum ||
        !memcmp(c->session, (uint8_t[16]){0}, 16u))
                              ...
                              uint64_t now, ninlil_power_apply apply, void *ctx)
{
    ...
    if (w) {
        if (!context_equal(&w->context, &p->context) ||
            ...
            w->closed_ms - w->first_tx_ms < NINLIL_METRIC_WINDOW * NINLIL_METRIC_RESPONSE_MS ||
            !w->airtime_sum_us || w->airtime_sum_us > NINLIL_METRIC_WINDOW * 400000u ||
            w->queue_max_us > 30000000u ||
            now-w->first_tx_ms > NINLIL_METRIC_MAX_AGE_MS)
            return NINLIL_ERR_CONFLICT;
        if (p->last.last_sequence && w->first_sequence <= p->last.last_sequence) {
            if (!same(w, &p->last))
                return NINLIL_ERR_CONFLICT;
        } else {
