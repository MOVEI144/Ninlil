...
int ninlil_radio_feedback_open(ninlil_radio_feedback *o, int8_t minimum,
                               int8_t maximum)
{
    ...
    if (!p->confirmed) {
        ...
        if (!p->metrics.pending) {
            ...
            if (rc != NINLIL_OK && rc != NINLIL_ERR_EMPTY)
                return rc;
            rc = ninlil_power_policy_plan(
                &p->policy, rc == NINLIL_OK ? &w : NULL, now, &o->proposed);
            if (rc != NINLIL_OK)
                return rc;
        }
    }
                                 ...
                                 uint32_t airtime, uint32_t queue)
{
    ...
    p = &o->peers[o->slot];
    changed = !p->confirmed || p->generation != o->proposed.context.generation;
    if (!p->confirmed) {
                                ...
                                uint64_t now)
{
    ninlil_feedback_peer *p;
    int slot, rc;
    if (!o || !o->opened || o->fault || !peer || !session || now < o->now_ms)
        return NINLIL_ERR_STATE;
...
int ninlil_radio_feedback_invalidate(ninlil_radio_feedback *o, uint16_t peer,
                                     uint64_t now)
{
