...
static int registered(const ninlil_search_node *n)
{
    return n->address && n->address != UINT16_MAX && n->membership_epoch &&
           n->role >= NINLIL_ROLE_BATTERY_LEAF && n->role <= NINLIL_ROLE_SITE_GATEWAY &&
...
int ninlil_route_search_begin(ninlil_route_search *s, const ninlil_search_config *c)
{
    if (!s || !c || !c->nodes || !c->edges || !c->validate ||
        !c->current_generation || !c->generation ||
        *c->current_generation != c->generation || c->node_count < 2u ||
        c->node_count > NINLIL_SEARCH_NODES || c->edge_count > NINLIL_SEARCH_EDGES ||
        !c->max_age_ms || c->max_age_ms > 120000u || !c->work_limit ||
        c->work_limit > 4096u || c->source >= c->node_count ||
        c->target >= c->node_count || c->source == c->target ||
    ...
    for (unsigned int i = 0u; i < c->edge_count; i++) {
        ...
        if (e->from >= c->node_count || e->to >= c->node_count || e->from == e->to ||
            !e->profile || (e->known & ~NINLIL_SEARCH_KNOWN_ALL) ||
...
static void expand(ninlil_route_search *s)
{
    ...
    if (e->from != p.nodes[p.count-1u] || e->to == c->excluded ||
        !member(&c->nodes[e->to]) ||
        e->known != NINLIL_SEARCH_KNOWN_ALL || e->attempts < 8u || !e->delivered ||
        !e->exchange_us || e->observed_ms > c->now_ms ||
    ...
    cost = ((uint64_t)e->exchange_us * e->attempts + e->delivered - 1u) /
           e->delivered + e->queue_us + e->wake_us + e->commit_us;
...
int ninlil_route_search_result(const ninlil_route_search *s, ninlil_search_result *out)
{
    ...
    for (unsigned int pick = 0u; pick < 3u && pick < s->candidate_count; pick++) {
        ...
        for (unsigned int i = 0u; i < s->candidate_count; i++) {
            ...
            if (best == NINLIL_SEARCH_CANDIDATES || shared < least ||
                (shared == least && compare(s, &s->candidates[i], &s->candidates[best]) < 0)) {
                least = shared;
                best = i;
            }
        }
