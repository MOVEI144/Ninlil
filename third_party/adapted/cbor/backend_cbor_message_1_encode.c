...
static bool ninlil_decoder_encode_message_1(zcbor_state_t *state, void *data)
{
    return encode_message_1(state, data);
}
		...
		zcbor_state_t *state, const struct EAD_1 *input)
{
	...
	bool tmp_result = (zcbor_multi_encode_minmax(1, 3, &(*input).EAD_1_count, ninlil_encoder_encode_ead_x, state, (&(*input).EAD_1), sizeof(struct ead_x)));
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)input, payload_len_out, states,
		ninlil_decoder_encode_message_1, sizeof(states) / sizeof(zcbor_state_t), 10);
}
