 ...
 * SF/channel alone is NOT sufficient. Recovery slots reserve the entire shared schedule.
 * Profiles must already be approved externally; the driver remains the final
 * legal-RF gate. This validator never touches a radio or grants a lease. */
 ...
 * validate Root authority, epochs and clock bounds separately before activation.
 * Encoder/decoder buffers are borrowed for the call; reassembly input must not
 * alias its mutable state. Successful decoding or assembly grants no TX right. */
...
typedef int (*ninlil_phy_digest_verify)(void *ctx, const uint8_t *data,
                                       size_t length, const uint8_t digest[32]);
 ...
 * limit (four slots) and airtime quotas before calling. Not for EDHOC bootstrap.
 * Digest callback verifies canonical SHA-256 with a reviewed crypto backend.
 * Full reassembly is only a proposal; authority/lease/apply proofs remain gates.
 * Deadline is fixed on first fragment and never renewed by duplicates. */
void ninlil_phy_reassembly_open(ninlil_phy_reassembly *state);
int ninlil_phy_reassembly_push(ninlil_phy_reassembly *state, uint16_t verified_peer,
                               const uint8_t *frame, size_t length,
                               uint64_t now_ms, ninlil_time_quality time_quality,
                               ninlil_phy_digest_verify verify, void *verify_ctx);
#endif
