...
/* Borrowed by the node until close. One explicit execution owner, no
 * allocation, persistence, timers or authority. Each entry is telemetry, not
 * custody. A profile change requires pause/reopen before more RF; this first
 * integration keeps the receive PHY fixed and buckets actual changes in legacy
 * TX power. Open does not start a radio. Never reopen a live borrowed monitor.
 */
 ...
 * deferrals. Unknown or >30-second residence is excluded, never clamped to
 * zero. The caller counts incomplete driver results separately, not as RF
 * trials. */
...
int ninlil_probe_monitor_reply(ninlil_probe_monitor *monitor, uint16_t peer,
                               ...
                               uint64_t now_ms);
/* Finalizes expired response windows, then returns the latest eight CLOSED
 * trials. Routing windows overlap; each trial enters once, but a new finalized
 * trial refreshes routing without waiting another eight-probe interval. This is
 * NOT an independent power-policy window; those remain in link_metrics.window.
...
/* Sleep invalidates provisional and closed observations, not logical
 * deliveries. Sequence and generation high-water marks survive this boot-local
 * pause. */
int ninlil_probe_monitor_pause(ninlil_probe_monitor *monitor, uint64_t now_ms);
/* Invalidate one peer, or all when peer==0, without inventing losses.
 * Caller must use its monotonic clock. Payload ownership is not touched. */
int ninlil_probe_monitor_invalidate(ninlil_probe_monitor *monitor,
