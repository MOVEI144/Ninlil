...
- P0 host tests cover store-before-receipt, restart handoff, duplicate, lost, pre-attempt, and reordered receipts, archive pressure, saturated receive/outbound phases, three-class receipt progress across repeated exact bound windows, per-class entry rotation, policy-error classification, durable permanent-rejection tombstone replay/conflict/capacity/corruption, CRITICAL preservation of CONTROL reserve across restart, starvation, custody replay including zero-byte tokens, Leaf windows, multi-Gateway dedupe and replay overlap, route epochs, group waves, capability-bitset forward compatibility, and authorization-before-inbox.
- Deadline matrices cover absent, failed, invalid, unavailable, runtime-only, before-deadline, and proven-expired clocks; unattempted local expiry; attempted ambiguity and retry suppression; unrelated traffic progress; invalid `APPLICATION_ACCEPTED` deadline contracts at API/wire/replay; live and replayed `EXPIRED` validation; and matching stored duplicate receipt recovery without a clock or after the deadline.
