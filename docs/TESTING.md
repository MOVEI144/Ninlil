...
- P0 host tests cover store-before-receipt, restart handoff, duplicate, lost, pre-attempt, and reordered receipts, archive pressure, saturated receive/outbound phases, three-class receipt progress across repeated exact bound windows, per-class entry rotation, policy-error classification, reserves and starvation, custody replay including zero-byte tokens, Leaf windows, multi-Gateway dedupe and replay overlap, route epochs, group waves, capability-bitset forward compatibility, and authorization-before-inbox.
