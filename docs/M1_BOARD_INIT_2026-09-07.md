...
### 後続の実機通信結果（18:29 JST追記）

上記はRF試験前の時点の記録。ユーザーが両モジュールの`R 201-250230`表示を確認した後、
[別のRF試験記録](M1_RF_CAMPAIGN_2026-09-07.md)の構成で実際に各方向1,000件を測定し、
両方向とも1,000応答・timeout 0・重複0を確認した。正式なアンテナ型式対応などの残件は
同記録へ引き継いだ。試験後は本記録と同じTX無効のinit-a-fix / init-bへ戻し、
書込みverify・fresh boot・各100回の初期化PASSを再確認した。
