...
- A missing receipt leaves the operation active; after a possible remote effect it may become `UNKNOWN` only through an explicit recovery decision. Deadline expiry cannot turn an attempted operation into inferred failure, and unavailable restart-safe time cannot authorize deadline-bearing first admission, DATA transmission, or a pending `APPLICATION_ACCEPTED` handoff. A matching duplicate of an already satisfied or terminal durable contract may re-arm one bounded receipt without new time evidence because it creates no new ownership decision.
