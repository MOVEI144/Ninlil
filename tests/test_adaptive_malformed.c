...
int main(void)
{
    ...
    good.operation[0]=1u;good.digest[0]=1u;good.authority_epoch=good.plan_epoch=1u;
    good.profile=good.profile_version=1u;good.activate_ms=1000u;good.expires_ms=60000u;
    good.count=1u;good.length=88u;
    CHECK(ninlil_phy_fragment_encode(&good,base,sizeof(base),&base_size)==0 && base_size==184u);
    for (unsigned int iteration=0u;iteration<20000u;iteration++) {
        ...
        if (iteration%2u) {
            ...
            frame[random32(&seed)%base_size]^=(uint8_t)(1u+random32(&seed)%255u);
        } else {
            size=random32(&seed)%257u;
            for (size_t i=0u;i<sizeof(frame);i++) frame[i]=(uint8_t)(random32(&seed)>>24);
        }
        ...
        if (rc==NINLIL_OK) {
            size_t encoded=0u;
            accepted++;
            CHECK(ninlil_phy_fragment_encode(&output,roundtrip,sizeof(roundtrip),&encoded)==0);
    ...
    CHECK(metrics.sequence==1u && !metrics.pending && !metrics.consecutive_losses);
    CHECK(ninlil_link_metrics_tick(&metrics,10000u)==0 && !metrics.attempts);
    printf("seed=20260909 mutated decoder accepted=%u rejected=%u / sleep invalidation PASS\n",accepted,rejected);
    return 0;
}
