...
int main(void)
{
    ...
    /* Restart with the original sender ID still owned after freeing quota. */
    ninlil_close(a);
    ninlil_close(b);
    test_link_init(&link, 320u);
    open_runtime(&a, a_path, 1u, &link, 0u, &policy, &rng_a);
    ...
    REQUIRE(ninlil_query(a, &second, &info) == NINLIL_OK &&
            info.outcome == NINLIL_OUTCOME_SATISFIED);
    REQUIRE(ninlil_receive(b, &inbound) == NINLIL_OK);
    REQUIRE(memcmp(inbound.message_id.bytes, second.bytes, NINLIL_ID_BYTES) ==
            0);
    puts(
        "temporary service capacity retries same durable ID after reboot PASS");
    ninlil_close(a);
    ninlil_close(b);
