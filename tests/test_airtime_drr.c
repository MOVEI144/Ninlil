...
static int class_fairness(void)
{
    ...
    for (unsigned int c = 0; c < 4; c++)
        CHECK(enqueue(&s, c + 1, (uint16_t)(c + 2), c, costs[c]) == 0);
    for (unsigned int i = 0; i < 12020; i++) {
        unsigned int c;
        now += 1000000;
        CHECK(ninlil_airtime_next(&s, now, &j) == 0);
        c = (unsigned int)j->traffic;
        service[c] += j->airtime_us;
        frames[c]++;
    ...
    for (unsigned int c = 0; c < 4; c++) {
        ...
        uint64_t delta = normalized > service[3] ? normalized - service[3]
                                                 : service[3] - normalized;
        CHECK(delta <= 400000);
    }
    printf("DRR airtime=%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64
           " frames=%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 "\n",
           service[0], service[1], service[2], service[3], frames[0], frames[1],
           frames[2], frames[3]);
    return 0;
}
static int reservation(void)
{
    ...
    for (unsigned int i = 0; i < 3; i++)
        CHECK(ninlil_airtime_next(&s, 0, &j) == NINLIL_ERR_EMPTY);
    CHECK(s.waiting);
...
static int peer_fairness(void)
{
    ...
    for (unsigned int i = 0; i < 410; i++) {
        ...
        do {
            now += 1000000;
            rc = ninlil_airtime_next(&s, now, &j);
        } while (rc == NINLIL_ERR_EMPTY);
        CHECK(rc == 0);
        peer = j->peer;
        cost = j->airtime_us;
        if (peer == 2)
...
int main(void)
{
    CHECK(class_fairness() == 0);
    CHECK(reservation() == 0);
    CHECK(peer_fairness() == 0);
    CHECK(bounds() == 0);
    puts("DRR regression PASS");
    return 0;
}
