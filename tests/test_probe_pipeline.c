...
int ninlil_node_recovery_receive(ninlil_node *n, uint16_t peer,
                                 node_control_kind k, const uint8_t *data,
                                 size_t size)
{
    (void)n;
    (void)peer;
...
int ninlil_secure_inspect_tx(const ninlil_secure_session *s,
                             const uint8_t *frame, size_t size, uint8_t *out,
                             size_t capacity, size_t *written)
{
    if (!s->ready || size <= 40u || size > 240u || size - 40u > capacity ||
        memcmp(frame + 8, s->material.fingerprint, 16u))
        return NINLIL_ERR_UNAUTHORIZED;
    memcpy(out, frame + 40, size - 40u);
    *written = size - 40u;
    return NINLIL_OK;
}
void ninlil_secret_clear(void *data, size_t size)
{
    memset(data, 0, size);
}
...
void ninlil_edhoc_close(void *e)
{
    (void)e;
}
void ninlil_lease_invalidate(void *c)
{
    (void)c;
}
...
int ninlil_node_receive(ninlil_node *n, const uint8_t *f, size_t length,
                        uint64_t now)
{
    (void)n;
    (void)f;
...
int ninlil_node_frame_current(ninlil_node *n, const uint8_t *f, size_t length,
                              uint64_t now)
{
    n->now_ms = now;
    return ninlil_node_probe_current(n, f, length);
}
...
int ninlil_routed_receive(ninlil_routed *r, const uint8_t *f, size_t length,
                          uint64_t now)
{
    (void)r;
    (void)f;
...
int ninlil_routed_frame_current(ninlil_routed *r, const uint8_t *f,
                                size_t length)
{
    (void)r;
    (void)f;
...
int ninlil_sx1262_radio_airtime(ninlil_sx1262_radio *r, uint16_t size,
                                uint32_t *airtime)
{
    (void)r;
    (void)size;
...
int ninlil_sx1262_radio_receive(ninlil_sx1262_radio *r, uint8_t *f,
                                uint16_t capacity, uint16_t *size,
                                ninlil_sx1262_rx_info *info, unsigned int wait)
{
    (void)r;
    (void)f;
    (void)capacity;
...
int ninlil_sx1262_radio_send(ninlil_sx1262_radio *r, const uint8_t *f,
                             uint16_t size)
{
    (void)f;
    (void)size;
    sends++;
    if (driver_result != NINLIL_OK)
...
{
    memset(n, 0, sizeof(*n));
    n->config.local = local;
    n->config.root = 1u;
    n->config.member_count = 2u;
    n->config.permitted_profile = 1u;
    n->local_index = (uint16_t)(local - 1u);
    n->root_index = 0u;
    n->joined = 1u;
...
static void probe(ninlil_node *n, uint8_t frame[240], uint64_t token)
{
    ...
    ninlil_node_put(frame + 4, 2u, 2u);
    ninlil_node_put(frame + 6, 1u, 2u);
    memcpy(frame + 8, session, 16u);
    frame[31] = 2u;
    frame[40] = NODE_PROBE;
    ninlil_node_put(frame + 41, token, 8u);
    n->peers[0].probe_token = token;
    n->peers[0].probe_sent = 0u;
}
int main(void)
{
    ...
    for (uint64_t i = 0u; i < 8u; i++) {
        ...
        clock_us = queued;
        child.now_ms = (uint64_t)clock_us / 1000u;
        probe(&child, frame, i + 1u);
        CHECK(ninlil_esp_network_emit(&pump, 1u, NINLIL_TRAFFIC_NORMAL, frame,
                                      240u) == 0);
        clock_us += 20000;
        ...
        CHECK(radio.requested_power_dbm ==
              -3); /* Must not be the measured value. */
        if (i >= 2u) {
            child.now_ms += 2500u;
            ninlil_node_put(reply, i + 1u, 8u);
            CHECK(ninlil_node_link_receive(&child, 1u, NODE_PROBE_REPLY, reply,
                                           8u) == 0);
            /* This boundary test supplies authentication explicitly. Actual
             * NS ingress and rejection are covered by feedback_pump tests. */
            CHECK(ninlil_probe_monitor_reply(pump.monitor, 1u, session, i + 1u,
                                             child.now_ms) == 0);
        }
        child.peers[0].probe_at = UINT64_MAX;
    }
    child.now_ms = 32509u;
    child.peers[0].report_at = 0u;
    ...
    CHECK(ninlil_node_link_receive(&root, 2u, NODE_OBSERVATION, report, 41u) ==
          0);
    CHECK(received_edge.queue_us == 500000u &&
          received_edge.airtime_us == 10000u);
    ...
    CHECK(ninlil_node_link_receive(&child, 1u, NODE_OBSERVATION_ACK, ack,
                                   11u) == 0);
    CHECK(ninlil_probe_monitor_read(pump.monitor, 1u, session, child.now_ms, 1,
                                    &w) == NINLIL_ERR_EMPTY);
    CHECK(ninlil_probe_monitor_read(pump.monitor, 1u, session, child.now_ms, 0,
                                    &w) == 0);
    ...
    CHECK(ninlil_probe_monitor_read(pump.monitor, 1u, session, 90000u, 0, &w) ==
          NINLIL_ERR_EMPTY);
    printf("production pump -> TX_DONE -> authenticated-reply boundary -> node "
           "report -> Coordinator input PASS; TX=%d queue_us=%u\n",
           sends, received_edge.queue_us);
    return 0;
}

/* This fixture tests IO/observation only. The real planner/Coordinator runs in
 * test_route_optimizer.c with its canonical production declarations. */
