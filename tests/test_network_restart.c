...
static void abort_committed(void)
{
    ...
    REQUIRE(ninlil_coordinator_abort(&f.coordinator) == NINLIL_ERR_STATE);
    REQUIRE(ninlil_coordinator_stage(&f.coordinator, &next, 102u, 50100u,
                                     NINLIL_TIME_RESTART_SAFE) ==
            NINLIL_ERR_STATE);
    REQUIRE(ninlil_coordinator_withdraw(&f.coordinator, first, 103u,
                                        NINLIL_TIME_RESTART_SAFE,
                                        0) == NINLIL_ERR_STATE);
    REQUIRE(ninlil_coordinator_withdraw(&f.coordinator, first + 1u, 50000u,
                                        NINLIL_TIME_RESTART_SAFE,
                                        0) == NINLIL_ERR_STATE);
    REQUIRE(ninlil_coordinator_withdraw(&f.coordinator, first, 50000u,
    ...
    REQUIRE(ninlil_coordinator_withdraw(&f.coordinator, first, 50000u,
                                        ...
                                        0) == NINLIL_OK);
    replay(&f);
    REQUIRE(f.coordinator.pending.epoch == 0u);
    puts("partially applied committed plan retains its fence through expiry "
         "PASS");
}
static void restore_wrong_active(void)
{
    ...
    for (unsigned int i = 0u; i < replacement.count; i++)
        REQUIRE(ninlil_coordinator_prepared(&f.coordinator,
                                            replacement.nodes[i],
                                            pending) == NINLIL_OK);
    REQUIRE(ninlil_coordinator_activate(&f.coordinator, 1001u,
                                        NINLIL_TIME_RESTART_SAFE,
    ...
    REQUIRE(ninlil_coordinator_activate(&f.coordinator, 1001u,
                                        NINLIL_TIME_RESTART_SAFE,
                                        1) == NINLIL_OK);
    puts("replayed flow-specific lease fence requires release PASS");
}
static void restore_partial_application(void)
{
    ...
    REQUIRE(ninlil_coordinator_route_check(&f.coordinator, &path, epoch,
                                           200u) == NINLIL_OK);
    ninlil_coordinator_disconnect(&f.coordinator, 2u);
    REQUIRE(ninlil_coordinator_route_check(&f.coordinator, &path, epoch,
                                           200u) != NINLIL_OK);
    REQUIRE(ninlil_coordinator_reconcile(&f.coordinator, 2u, epoch) ==
            NINLIL_OK);
    REQUIRE(ninlil_coordinator_route_check(&f.coordinator, &path, epoch,
                                           200u) != NINLIL_OK);
    REQUIRE(ninlil_coordinator_reconcile(&f.coordinator, 1u, epoch) ==
            NINLIL_OK);
    REQUIRE(ninlil_coordinator_route_check(&f.coordinator, &path, epoch,
                                           200u) == NINLIL_OK);
    puts("partial application replay requires all fresh confirmations PASS");
}
int main(void)
{
    abort_committed();
