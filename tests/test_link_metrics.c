...
int main(void)
{
    ...
    ninlil_link_context c={.generation=1,.profile=1,.power_dbm=-3,.session={1}};
    ninlil_link_metrics_open(&m);
    for(uint64_t i=0;i<8;i++) {
        uint64_t now=i*4000+1;
        CHECK(ninlil_link_metrics_tx(&m,&c,i+1,now,100000,(uint32_t)i*1000)==0);
        before=m;
        CHECK(ninlil_link_metrics_tx(&m,&c,i+1,now,100000,(uint32_t)i*1000)==0);
        CHECK(!memcmp(&m,&before,sizeof(m)));
        CHECK(ninlil_link_metrics_read(&m,now,&w)==NINLIL_ERR_EMPTY);
        CHECK(ninlil_link_metrics_reply(&m,99,now+1)==NINLIL_ERR_NOT_FOUND);
    ...
    CHECK(w.attempts==8 && w.delivered==8 && w.first_sequence==1 && w.last_sequence==8);
    ...
    CHECK(ninlil_link_metrics_tick(&m,43000)==0 && m.consecutive_losses==1);
    CHECK(ninlil_link_metrics_tx(&m,&c,11,44000,10000,0)==0);
    CHECK(ninlil_link_metrics_tick(&m,47000)==0 && m.consecutive_losses==2);
    c.session[0]=2;
    CHECK(ninlil_link_metrics_tx(&m,&c,12,48000,10000,0)==0);
    CHECK(m.attempts==0 && m.consecutive_losses==0);
    before=m;
    CHECK(ninlil_link_metrics_tx(&m,&c,13,48001,10000,0)==NINLIL_ERR_BUSY);
    CHECK(!memcmp(&m,&before,sizeof(m)));
    CHECK(ninlil_link_metrics_tick(&m,47000)==NINLIL_ERR_INVALID);
    CHECK(!memcmp(&m,&before,sizeof(m)));
    CHECK(ninlil_link_metrics_tx(&m,&c,13,UINT64_MAX,10000,0)==NINLIL_ERR_INVALID);
    CHECK(!memcmp(&m,&before,sizeof(m)));
    ...
    m.sequence=UINT64_MAX; before=m;
    CHECK(ninlil_link_metrics_tx(&m,&c,15,204000,10000,0)==NINLIL_ERR_CAPACITY);
    CHECK(!memcmp(&m,&before,sizeof(m)));
    puts("closed non-overlapping probe windows/context/freshness/boundaries PASS");
    return 0;
}
